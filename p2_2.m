...
a=innum(2);
fun=@(t,x)[x(3) x(4) (F_z*cos(w*t(1))+eta_d*abs(x(4)-x(3))^a*(x(4)-x(3))+...
    k_F*(x(2)-x(1))-eta_z*x(3)-pho*g*R_o^2*pi*x(1))/(m_o+m_z_add) ...
    -(eta_d*abs(x(4)-x(3))^a*(x(4)-x(3))+k_F*(x(2)-x(1)))/m_i]';
...
P=sum(abs(X(4,restep)-X(3,restep)).^(2+a))*step*eta_d/80;